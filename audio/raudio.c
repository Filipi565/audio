...
#include <raudio.h>         // Declares module functions
...
//----------------------------------------------------------------------------------
AudioData AUDIO = {          // Global AUDIO context
...
Wave LoadWave(const char *fileName)
{
    ...
    if (fileData != NULL) wave = LoadWaveFromMemory(GetFileExtension(fileName), fileData, dataSize, &a);
...
Wave LoadWaveFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int *err_code)
{
    Wave wave = { 0 };
    *err_code = 0;
